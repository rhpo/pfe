...
| `pfe\_code` | Deterministic PFE code generator |
...
\- `teachers.id` / `students.id` are domain entity IDs (used by foreign keys in `defense\_juries`, `jury\_grades`, etc.)
...
cp .env.example .env       # set JWT\_SECRET, RESEND\_API\_KEY, etc.
...
| `JWT\_SECRET` | HMAC key for signing tokens | Yes |

| `RESEND\_API\_KEY` | Resend key for email notifications | No |

| `DB\_PATH` | SQLite file path | No (defaults to `./pfe.db`) |
